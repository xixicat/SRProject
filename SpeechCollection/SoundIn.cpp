...
bool CSoundIn::Start(WAVEFORMATEX* format)
{
	...
	else
	{
		...
		if(mmReturn)
		{
			waveInErrorMsg(mmReturn, "in Start() step 1");
			return FALSE;
		}
		else
		{
			...
			if(mmReturn )
			{
				waveInErrorMsg(mmReturn, "in Start() step 2");
				return FALSE;
			}
			m_bRecording = true;
		}
...
// 		if(mmReturn)
// 		{
// 			waveInErrorMsg(mmReturn, "in Stop() step 1");
// 			return;
// 		}
// 		else
// 		{
...
// 			if(mmReturn) waveInErrorMsg(mmReturn, "in Stop() step 2");
// 		}
...
void CSoundIn::OnMM_WIM_DATA(UINT parm1, LONG parm2)
{
	...
	if(mmReturn)
	{
		//waveInErrorMsg(mmReturn, "in OnWIM_DATA() step 1");
		return;
	}
	...
	if(m_bRecording)
	{
		CBuffer buf(pHdr->lpData, pHdr->dwBytesRecorded);
		...
		if(mmReturn)
		{
			//waveInErrorMsg(mmReturn, "in OnWIM_DATA() step 2");
		}
		else // no error
		{
			// add the input buffer to the queue again
			mmReturn = ::waveInAddBuffer(m_hRecord, pHdr, sizeof(WAVEHDR));
			if(mmReturn) waveInErrorMsg(mmReturn, "in OnWIM_DATA() step 3");
	...
	InterlockedDecrement(&m_QueuedBuffers);
	delete pHdr->lpData;
	delete pHdr;
}
...
int CSoundIn::AddInputBufferToQueue()
{
	...
	if(mmReturn)
	{
		waveInErrorMsg(mmReturn, "in AddInputBufferToQueue() step 1");
	...
	if(mmReturn)
	{
		waveInErrorMsg(mmReturn, "in AddInputBufferToQueue() step 2");
		return m_QueuedBuffers;
	}

	// no error
	// increment the number of waiting buffers
	return InterlockedIncrement(&m_QueuedBuffers);
}
